...
export default function CategoryManager({ categories, onSaved, onDeleted }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-6">
      ...
      {error && (
        <div
          className="flex items-center gap-2 bg-red-50 text-red-700 border border-red-200 rounded-2xl px-5 py-4 text-sm mb-6"
          role="alert"
        >
      ...
      <div className="space-y-3">
        {categories.map((cat) => (
          <div
            key={cat.id}
            className="flex flex-wrap items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-200 transition-colors"
          >
            {editingId === cat.id ? (
        ...
        {categories.length === 0 && <p className="text-center text-sm text-gray-500 py-4">{t('admin.noReports')}</p>}
      </div>
    </div>
  );
}
